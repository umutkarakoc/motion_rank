...
async fn get_project(
    ...
    layout::page(
        ...
        html! {
            ...
            div class="is-flex flex1 " {
                div class="mr-2 mb-0 p-4 is-flex-direction-column is-flex is-align-items-stretch has-background-light"
                    style="width: 240px;" {
                    ...
                    div class="is-flex is-align-items-center is-justify-content-space-between mt-4" {
                        h5 class="card-header-title p-0 has-text-dark" { "Projects" }
                    }
                    div class="is-flex is-flex-direction-column mt-2" id="projects" {

                        @for project in projects.iter() {
                            a href={"/project/"(project.id.to_string())} hx-boost="true" hx-target="body" hx-swap="outerHTML"
                                class={"button mb-2 is-light " (if project_id == project.id {"is-link"} else {""}) }
                                { (project.name) }
                        }

                        button class="button is-light is-primary" hx-target="body" hx-post="/project" hx-push-url="true" hx-swap="outerHTML" {"Create New Project"}
                    }
                    @if shared_projects.len() > 0 {
                        div id="shared_project_title" class="is-flex is-align-items-center mt-4 is-justify-content-space-between" {
                            h5 class="card-header-title p-0 has-text-dark" { "Shared Projects" }
                        }
                        div class="is-flex is-flex-direction-column mt-2" id="shared_projects" {
                            @for project in shared_projects.iter() {
                                a href={"/project/"(project.id.to_string())} hx-boost="true" hx-target="body" hx-swap="outerHTML"
                                    class={"button is-info mb-2 " (if project_id == project.id {""} else {"is-light"}) }
                                { (project.name) }
                            }
                        }
                    }
                ...
                div class="is-flex is-flex-direction-column p-4" style="flex: 1"{
                    div class="is-flex is-flex-direction-row is-justify-content-space-between" {
                        div class="is-flex"{
                            p class="title is-size-3 m-0" {"Project"}
                            div id="project_name" class="ml-2 is-flex" {
                                p class="title is-size-3 m-0 has-text-grey" { (title) }

                                @if owner == user_id {
                                button class="button is-white" hx-get={"/project/"(project_id.to_string())"/edit"} hx-target="#project_name" hx-swap="outerHTML" {
                                    span class="icon has-text-grey" { i class="fa-solid fa-pen-to-square"{} } }
                                }
                        ...
                        div class="field is-grouped" {
                            input id="search" class="input placeholder" style="width: 280px;" type="text"
                                placeholder="Search videos" hx-push-url="true" hx-get={"/project/"(project_id.to_string())}
                                name="search" hx-target="body" hx-swap="outerHTML" hx-preserve="true" {}
                            div id="sharing" {}

                            @if owner == user_id { (access::render_access(project_id, user_id, &db).await) }

                            a class="button is-light upload ml-2" hx-post={"/project/"(project_id.to_string())"/upload"} hx-target="body" hx-swap="none" {
                                span class="icon is-small" { i class="fas fa-video" {} }
                                span {"Upload"}
                            }

                            button hx-get="/profile/notification" class="button is-light ml-2" {
                                i class="fa-solid fa-bell"{}
                            }

                            div class="dropdown is-hoverable ml-2" {
                                div class="dropdown-trigger" {
                                    button hx-get="/profile/menu" class="button is-light " {
                                ...
                                div class="dropdown-menu" id="dropdown-menu3" role="menu" {
                                    div class="dropdown-content" {
                                        ...
                                        a href="/auth/logout" class="dropdown-item button is-danger is-inverted" {"Logout"}
                                    }
                                }
                            }
                        }
                    ...
                    div id="page" class="is-flex is-flex-direction-column" style="flex: 1; overflow-y: auto;" {
                        div class="is-flex is-flex-wrap-wrap is-align-content-flex-start mt-6 flex1" {
                            (videos)
                        }
                    }
...
async fn create_project(
    ...
    let project = sqlx::query_as!(
        Project,
        r#"insert into project (user_id, name) VALUES ($1, 'New Project') returning *"#,
        user_id
    )
    .fetch_one(&db)
    .await
...
async fn edit_project(
    ...
    html! { div class="ml-2 is-flex" {
        input name="name" autofocus style="height:38px; border: none" class="input is-size-3 p-0 is-primary is-shadowless
                has-text-grey title" hx-trigger="blur"
                hx-patch={"/project/"(project.id.to_string())} hx-push-url="true" hx-swap="outerHTML" hx-target="body" value=(project.name) {}
        }
    }.into_string()
    .into_response()
}
